...
# Função para mostrar arquivos do /boot
show_boot_files() {
...
# Detecta o subvolume usado como raiz atualmente
...
adiciona_boot_stanza() {
    ...
    } || warning "refind_linux.conf não encontrado."
    echo
...
# Nova função: Configurar refind-btrfs
configure_refind_btrfs() {
    ...
    # --- Seção: snapshot-search (primeiro bloco) ---
    ...
    # --- Seção: boot-stanza-generation.icon ---
...
# Restaura backup do refind-btrfs.conf
...
# Função para editar o refind.conf com o nano
editar_refind_conf() {
    busca_refind_conf
...
# Função para editar o refind-btrfs.conf com o nano
...
menu_principal() {
    ...
    ask "Escolha uma opção [1-7]:" opcao
}
