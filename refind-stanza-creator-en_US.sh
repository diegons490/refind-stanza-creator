...
add_boot_stanza() {
    ...
    show_partuuids
    show_root_partition
    ask "Enter PARTUUID of Btrfs partition:" PARTUUID_BTRFS
    list_subvolumes
    detect_current_subvolume
    ask "Enter subvolume name (ex: @ or leave empty if using subvolid=5): " SUBVOLUME
    show_boot_files
    ask "Enter kernel filename:" KERNEL_FILE
    ask "Enter initrd filename:" INITRD_FILE
    LOADER_PATH="${KERNEL_FILE}"
    INITRD_PATH="${INITRD_FILE}"
    [ -f /boot/refind_linux.conf ] && {
        header "refind_linux.conf content"
        sed 's/^/  /' /boot/refind_linux.conf
    } || warning "refind_linux.conf not found."
    echo
    echo -e "${YELLOW}Example: quiet zswap.enabled=0 root=PARTUUID=xxxx-xxxx rw rootflags=subvol=@ quiet splash${NC}"
    echo
    ask "Enter kernel parameters without quotes:" KERNEL_OPTIONS
    list_icons
    ask "Enter icon path (ex: EFI/refind/icons/os_linux.png):" ICON_PATH
    ask "Enter OS name (menuentry):" MENU_NAME
...
main_menu() {
    ...
    ask "Choose an option [1-7]:" choice
}
...
while true; do
    ...
    case $choice in
        ...
        5) restore_refind_backup ;;
        6) restore_refind_btrfs_backup ;;
